...
OUTPUT_PATH = _ROOT / "data" / "eval_results_d_v32_cascade.json"
...
def main():
    print("=" * 70)
    print("Cascade L2 BERT 评测 - D_eval_set_v3.2 (1500 条黄金评测集)")
    print("=" * 70)
    ...
    print(f"  v3.6.1 (修复后):       {acc*100:.2f}% (P0 召回 {p0_recall*100:.2f}%)  [v3.6.1 P0 红线补丁]")
    print(f"  提升幅度:              +{acc*100-v360_before_fix:.2f}pp (整体) | +{p0_recall*100-v360_p0_before:.2f}pp (P0)")
    print(f"  注: v3.6.1 给 IR 加 11 类 D v3.2 P0 意图 (badcase_patches_v361.py),")
    print(f"      解决 safety_*/security_*/sys_service_* 在 IR 里的 label 缺失.")
    print(f"{'='*70}")

    # 保存
    output = {
        "eval_version": "v3.6.0-cascade-l2-on-D-v3.2",
