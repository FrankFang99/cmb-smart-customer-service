"""
v3.5.3 真 LLM 评测 - 1500 样本 (train/holdout 拆分) 验证 v3.5.1 修复是否泛化
=========================================================================

设计:
- 数据: evaluation_dataset_v6.0.json (1500 样本, 991 train + 509 holdout)
- 跑全部 1500 样本, 分别统计 train / holdout 的指标
- 关键: holdout 指标 = 真实泛化能力
- 对比 v3.4.0 (600 样本) 和 v3.5.2 (600 样本) 的 5.17pp, 看 1500 样本下是否依然显著

输入: data/evaluation_dataset_v6.0.json
输出: data/eval_results_v353.json
"""

from __future__ import annotations

import json
import sys
import time
from collections import Counter, defaultdict
from pathlib import Path
from typing import Dict, List, Any

_ROOT = Path(__file__).resolve().parents[1]
if str(_ROOT) not in sys.path:
    sys.path.insert(0, str(_ROOT))

from src.agent.e2e_pipeline import create_e2e_pipeline
from src.components.intent_recognizer import IntentRecognizer


DATASET_PATH = _ROOT / "data" / "evaluation_dataset_v6.0.json"
OUTPUT_PATH = _ROOT / "data" / "eval_results_v353.json"


def evaluate_sample(pipeline, sample: Dict) -> Dict:
    question = sample["question"]
    expected_intent = sample["intent"]
    is_p0_label = sample.get("is_p0", False)

    t0 = time.time()
    try:
        e2e = pipeline.handle(question, session_id=f"eval_{sample['id']}")
    except Exception as e:
        return {
            "sample_id": sample["id"],
            "question": question,
            "expected_intent": expected_intent,
            "is_p0_label": is_p0_label,
            "split": sample.get("split", "unknown"),
            "error": str(e),
            "intent_match": False,
            "p0_recall": False,
            "l0_compliance": False,
            "rag_hit": False,
            "transfer_correct": False,
            "elapsed_ms": 0,
        }
    t_total = (time.time() - t0) * 1000

    actual_intent = e2e.get("intent", "unknown")
    intent_match = actual_intent == expected_intent
    p0_recall = e2e.get("l0_triggered", False) if is_p0_label else None
    if e2e.get("l0_triggered"):
        l0_compliance = e2e.get("action") == "transfer_human" and "llm_elapsed_ms" not in e2e
    else:
        l0_compliance = None
    rag_hit = False
    if not e2e.get("l0_triggered"):
        rag_hit = len(e2e.get("sources", [])) > 0
    if is_p0_label:
        transfer_correct = e2e.get("action") == "transfer_human"
    else:
        transfer_correct = e2e.get("action") == "answer"

    return {
        "sample_id": sample["id"],
        "question": question,
        "expected_intent": expected_intent,
        "actual_intent": actual_intent,
        "is_p0_label": is_p0_label,
        "split": sample.get("split", "unknown"),
        "intent_match": intent_match,
        "p0_recall": p0_recall,
        "l0_compliance": l0_compliance,
        "rag_hit": rag_hit,
        "transfer_correct": transfer_correct,
        "action": e2e.get("action"),
        "l0_triggered": e2e.get("l0_triggered", False),
        "elapsed_ms": round(t_total, 1),
    }


def aggregate_split(results: List[Dict]) -> Dict:
    total = len(results)
    if total == 0:
        return {"total": 0}
    intent_correct = sum(1 for r in results if r.get("intent_match"))
    p0_samples = [r for r in results if r.get("is_p0_label")]
    p0_recall = (
        sum(1 for r in p0_samples if r.get("p0_recall")) / len(p0_samples)
        if p0_samples else 1.0
    )
    l0_samples = [r for r in results if r.get("l0_triggered")]
    l0_compliance = (
        sum(1 for r in l0_samples if r.get("l0_compliance")) / len(l0_samples)
        if l0_samples else 1.0
    )
    non_l0_samples = [r for r in results if not r.get("l0_triggered")]
    rag_hit = (
        sum(1 for r in non_l0_samples if r.get("rag_hit")) / len(non_l0_samples)
        if non_l0_samples else 1.0
    )
    transfer_correct = sum(1 for r in results if r.get("transfer_correct"))
    transfer_accuracy = transfer_correct / total

    by_intent_group: Dict[str, Dict] = defaultdict(lambda: {"total": 0, "correct": 0})
    for r in results:
        group = r.get("expected_intent", "unknown").split("_")[0]
        by_intent_group[group]["total"] += 1
        if r.get("intent_match"):
            by_intent_group[group]["correct"] += 1
    for g in by_intent_group:
        if by_intent_group[g]["total"]:
            by_intent_group[g]["accuracy"] = round(
                by_intent_group[g]["correct"] / by_intent_group[g]["total"], 4
            )

    return {
        "total": total,
        "intent_accuracy": round(intent_correct / total, 4),
        "p0_recall": round(p0_recall, 4),
        "l0_compliance": round(l0_compliance, 4),
        "rag_hit_rate": round(rag_hit, 4),
        "transfer_accuracy": round(transfer_accuracy, 4),
        "p0_samples": len(p0_samples),
        "l0_triggered": len(l0_samples),
        "by_intent_group": dict(by_intent_group),
    }


def main():
    print("=" * 80)
    print("v3.5.3 真 LLM 评测 - 1500 样本 (train/holdout 拆分) - 验证 v3.5.1 修复泛化")
    print("=" * 80)

    with open(DATASET_PATH, encoding="utf-8") as f:
        dataset = json.load(f)
    samples = dataset["samples"]
    print(f"数据集版本: {dataset['dataset_version']}, 总样本: {len(samples)}")
    print(f"train: {dataset['train_count']}, holdout: {dataset['holdout_count']}")
    print()

    pipeline = create_e2e_pipeline(k=3)
    intent_recognizer = IntentRecognizer()

    # 按 split 分组
    train_samples = [s for s in samples if s.get("split") == "train"]
    holdout_samples = [s for s in samples if s.get("split") == "holdout"]
    print(f"开始评测 train ({len(train_samples)}) + holdout ({len(holdout_samples)}) ...")

    all_results = []
    t0 = time.time()
    total_n = len(samples)
    for i, sample in enumerate(samples, 1):
        if i % 100 == 0 or i == 1:
            elapsed = time.time() - t0
            eta = (elapsed / i) * (total_n - i) if i > 0 else 0
            print(f"  [{i}/{total_n}] ({i/total_n*100:.0f}%)  已用 {elapsed:.0f}s, 预计剩余 {eta:.0f}s")
        all_results.append(evaluate_sample(pipeline, sample))
    total_elapsed = time.time() - t0
    print(f"\n  跑完 {total_n} 样本, 总耗时 {total_elapsed:.1f}s ({total_elapsed/60:.1f} 分钟)")

    # 分 split 统计
    train_results = [r for r in all_results if r.get("split") == "train"]
    holdout_results = [r for r in all_results if r.get("split") == "holdout"]
    overall = aggregate_split(all_results)
    train_summary = aggregate_split(train_results)
    holdout_summary = aggregate_split(holdout_results)

    print("\n" + "=" * 80)
    print("v3.5.3 真 LLM 评测结果 (train/holdout)")
    print("=" * 80)
    print(f"{'指标':<22} {'Overall':<12} {'Train (991)':<14} {'Holdout (509)':<14} {'Δ泛化':<10}")
    print("-" * 80)
    for metric in ("intent_accuracy", "p0_recall", "l0_compliance", "rag_hit_rate", "transfer_accuracy"):
        o = overall.get(metric, 0)
        t = train_summary.get(metric, 0)
        h = holdout_summary.get(metric, 0)
        delta = round(t - h, 4) if t and h else 0
        print(f"{metric:<22} {o*100:>6.2f}%      {t*100:>6.2f}%        {h*100:>6.2f}%        {delta*100:>+6.2f}pp")
    print()

    # 与 v3.4.0 / v3.5.2 对比
    print("=" * 80)
    print("v3.4.0 (600) / v3.5.2 (600) / v3.5.3 (1500) 对比")
    print("=" * 80)
    print(f"{'指标':<22} {'v3.4.0':<10} {'v3.5.2':<10} {'v3.5.3 Overall':<18} {'v3.5.3 Holdout':<18}")
    print("-" * 80)
    comparison = [
        ("intent_accuracy", 0.83, 0.8817, overall["intent_accuracy"], holdout_summary["intent_accuracy"]),
        ("p0_recall", 0.50, 0.50, overall["p0_recall"], holdout_summary["p0_recall"]),
        ("l0_compliance", 1.00, 1.00, overall["l0_compliance"], holdout_summary["l0_compliance"]),
        ("rag_hit_rate", 0.9875, 0.9875, overall["rag_hit_rate"], holdout_summary["rag_hit_rate"]),
    ]
    for name, v340, v352, v353_all, v353_h in comparison:
        print(f"{name:<22} {v340*100:>6.2f}%    {v352*100:>6.2f}%    {v353_all*100:>10.2f}%      {v353_h*100:>10.2f}%")

    # 泛化分析
    print("\n" + "=" * 80)
    print("泛化分析 (train vs holdout)")
    print("=" * 80)
    overfit_signals = []
    for metric in ("intent_accuracy", "p0_recall", "l0_compliance"):
        t = train_summary.get(metric, 0)
        h = holdout_summary.get(metric, 0)
        delta = (t - h) * 100
        if delta > 2:
            overfit_signals.append("  [WARN] %s: train %.2f%% vs holdout %.2f%%, 差 %+.2fpp (过拟合信号)" % (metric, t*100, h*100, delta))
        elif delta < -2:
            overfit_signals.append("  [OK] %s: holdout 比 train 高 %.2fpp (反常, 检查数据)" % (metric, -delta))
        else:
            overfit_signals.append("  [OK] %s: train/holdout 差 %+.2fpp (泛化 OK)" % (metric, delta))
    for s in overfit_signals:
        print(s)

    # 业务组 holdout 准确率
    print("\n按业务分组 (holdout):")
    for g, v in sorted(holdout_summary["by_intent_group"].items(), key=lambda x: -x[1]["total"]):
        print("  %-8s: %d/%d = %.1f%%" % (g, v["correct"], v["total"], v["accuracy"]*100))

    # 输出
    output = {
        "eval_version": "v3.5.3",
        "eval_date": time.strftime("%Y-%m-%dT%H:%M:%S"),
        "llm": "MiniMax-M2.7 via api.minimaxi.com/v1 (订阅 Key)",
        "dataset_version": "v6.0",
        "dataset": {
            "total": len(samples),
            "train": len(train_samples),
            "holdout": len(holdout_samples),
            "p0_count": sum(1 for s in samples if s.get("is_p0", False)),
        },
        "patches_applied": "v3.5.1 Badcase 修复 (L0 14 词 + 意图 8 条)",
        "summary": {
            "overall": overall,
            "train": train_summary,
            "holdout": holdout_summary,
        },
        "comparison_with_v340_v352": {
            "intent_accuracy": {
                "v3.4.0": 0.83, "v3.5.2": 0.8817,
                "v3.5.3_overall": overall["intent_accuracy"],
                "v3.5.3_holdout": holdout_summary["intent_accuracy"],
            },
            "p0_recall": {
                "v3.4.0": 0.50, "v3.5.2": 0.50,
                "v3.5.3_overall": overall["p0_recall"],
                "v3.5.3_holdout": holdout_summary["p0_recall"],
            },
        },
        "total_elapsed_ms": round(total_elapsed * 1000, 1),
    }
    OUTPUT_PATH.parent.mkdir(parents=True, exist_ok=True)
    with open(OUTPUT_PATH, "w", encoding="utf-8") as f:
        json.dump(output, f, ensure_ascii=False, indent=2, default=str)
    print(f"\n结果保存: {OUTPUT_PATH}")


if __name__ == "__main__":
    main()
