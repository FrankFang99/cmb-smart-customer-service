# -*- coding: utf-8 -*-
"""v3.10.1 v4 — 全量跑 (用更严格的 D v3.2 升级规则)"""
...
def _stable_trace_seed(query, pri):
    return f"tr_v3101v4_{pri}_" + hashlib.md5(query.encode('utf-8')).hexdigest()[:12]
...
print()
print('=' * 70)
print(f'v3.10.1 v4 全量结果 (耗时 {elapsed:.0f}s):')
print('=' * 70)
...
report = {
    ...
    "patches_applied": [
        ...
        "d_v32_dedup_v3101 v4: 18 条 真·P0 priority 升级 (含 礼貌询问过滤)",
        "v364 biz_transfer_large: 收紧 (删 怎么办/怎么操作/大额 单独)",
        "l0_v3101 large_amount: 收紧 (删 大额 单独, 保留 大额转/汇)",
    ],
}
report_path = _ROOT / 'data' / 'observable_v3101_v4_report.json'
report_path.write_text(json.dumps(report, ensure_ascii=False, indent=2), encoding='utf-8')
print(f'\n报告: {report_path}')

# P0 漏召回
print('\n=== P0 漏召回 (final_action != transfer_human) ===')
missed = cur.execute("""
    SELECT user_input, expected_action, final_action, final_intent
    FROM traces WHERE priority='P0' AND final_action!='transfer_human'
""").fetchall()
if not missed:
    print('  (无 — P0 100% 不破!)')
else:
    for q, exp, fa, fi in missed:
        print(f'  Q: {q} | exp={exp} | fa={fa} | fi={fi}')

# P1 错答 Top 5
print('\n=== P1 transfer_human 错答 Top 5 expected_action ===')
for r in cur.execute("""
    SELECT expected_action, COUNT(*) FROM traces
    WHERE priority='P1' AND final_action='transfer_human'
    GROUP BY expected_action ORDER BY 2 DESC LIMIT 5
"""):
    print(f'  {r[0]}: {r[1]} 条')
